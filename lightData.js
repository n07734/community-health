...
const slimValue = (value) => {
    const newValue = (typeof value === 'string' && value.length && value)
        || (/^[\d.]+$/.test(`${value}`) && /^[^0]/.test(`${value}`) && value)
        || (Array.isArray(value) && value.length && slimArray(value))
        || (value && Object.keys(value).length && slimObject(value))
